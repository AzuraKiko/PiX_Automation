...
public class BasePage {
    ...
    public String getElementTextByJS(String locatorType) {
        WebElement element = getDriver().findElement(By.xpath(locatorType)); // You can change the locator if necessary
    ...
    public String getElementTextByJS(String locatorType, String... dynamicValues) {
        WebElement element = getDriver().findElement(By.xpath(getDynamicXpath(locatorType, dynamicValues)));
        JavascriptExecutor js = (JavascriptExecutor) getDriver();
        return (String) js.executeScript("return arguments[0].innerText;", element);
    }
    ...
    public void clickToCheckbox(String locator, String checkboxLabelName) {
        waitForElementVisible(locator, checkboxLabelName);
        checkToDefaultCheckBoxOrRadio(locator, checkboxLabelName);
    }
    ...
    public static void sleepInSecond(double second) {
        try {
    ...
    public boolean isInputFieldBlank(String locator) {
        String value = getDriver().findElement(By.xpath(locator)).getAttribute("value");
    ...
    public boolean areElementsEnabled(String locators) {
        List<WebElement> elemments = getDriver().findElements(By.xpath(locators));
    ...
    public void sendKeysToElements(String locators, String textValue) {
        List<WebElement> elemments = getDriver().findElements(By.xpath(locators));
        for (WebElement element : elemments) {
            element.sendKeys(textValue);
        }
    ...
    public void clearThenSendKeyToElement(String locator, String value) {
        WebElement element = getDriver().findElement(By.xpath(locator));
        clearValueInElementByDeleteKey(locator);
        element.sendKeys(value);
    }

    public void selectFirstRecordOnTable(String locatorTable) {
        WebElement table = getDriver().findElement(By.xpath(locatorTable));
        //List<WebElement> rows = table.findElements(By.xpath("//tbody/tr"));
        //int randomIndex = new Random().nextInt(rows.size());
        //WebElement randomRow = rows.get(randomIndex);
        scrollToElement(locatorTable + "//child::tbody//child::tr[1]");
        //WebElement actionButton = randomRow.findElement(By.xpath("td["+randomRow+"]/button"));
        clickToElement(locatorTable + "//child::tbody//child::tr[1]");
    }

    public static String getCurrentDate() {
        // Tạo đối tượng SimpleDateFormat với định dạng "dd MMM yyyy"
        SimpleDateFormat formatter = new SimpleDateFormat("dd MMM yyyy");

        // Lấy ngày hiện tại
        Date currentDate = new Date();

        // Định dạng ngày hiện tại theo định dạng yêu cầu
        return formatter.format(currentDate) + " 06:30:";
    }
    ...
    public List<String> getValueFromTable(String locator) {
        List<WebElement> columnElements = getDriver().findElements(By.xpath(locator));

        // Tạo mảng để lưu giá trị của cột
        List<String> columnData = new ArrayList<>();

        // Lặp qua các hàng trong cột và lấy giá trị
    ...
    public void clearTextAreaByJS(String locatorType) {
        WebElement textArea = getDriver().findElement(By.xpath(locatorType));
        JavascriptExecutor js = (JavascriptExecutor) getDriver();
        js.executeScript("arguments[0].value='';", textArea); // Xóa nội dung của textarea qua JavaScript
    }
